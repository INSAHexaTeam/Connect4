...
:- module(minimax_poids_colonnes, [simuler_coup_poids_colonnes/4, choisir_colonne_minimax_poids_colonnes/2]).
...
choisir_colonne_minimax_poids_colonnes(Plateau, Colonne) :-
    minimax_poids_colonnes(Plateau, 3, true, _, Colonne).  % Depth of 3

% Minimax algorithm
% minimax(+Plateau, +Profondeur, +MaximizingPlayer, -MeilleurScore, -MeilleurCoup)
minimax_poids_colonnes(Plateau, 0, _, Score, _) :-
    evaluer_plateau(Plateau, Score).  % Base case: evaluate the board

minimax_poids_colonnes(Plateau, Profondeur, true, MeilleurScore, MeilleurCoup) :-
...
minimax_poids_colonnes(Plateau, Profondeur, false, MeilleurScore, MeilleurCoup) :-
    findall(Coup, joueur_peut_jouer(Coup), Coups),  % Use joueur_peut_jouer/1 from joueurs module
    Profondeur1 is Profondeur - 1,
...
minimax_score(Plateau, Profondeur, MaximizingPlayer, Coup, Score) :-
    simuler_coup_poids_colonnes(Plateau, Coup, _, NouveauPlateau),
    minimax_poids_colonnes(NouveauPlateau, Profondeur, MaximizingPlayer, BaseScore, _),
